...
class Scraper():
    ...
    def get_data_using_id(self, file_name, start_id, end_id):
        self.all_ids = list(pd.read_csv("{}".format(file_name)).iloc[:,0])
        self.required_ids = self.all_ids[start_id : end_id]
        self.error_ids=[]
    
        self.filename = "{}-{}-data.csv".format(start_id, end_id)

        for i in range(len(self.required_ids)):

            if(len(str(self.required_ids[i])) != 8):
                self.s = ""
                for j in range(8-len(str(self.required_ids[i]))):
                    self.s+="0"
                    self.s+=str(self.required_ids[i])

                url = "https://www.pagesjaunes.fr/pros/" + self.s
                try:
                    self.extract_details('{}'.format(url))
                except Exception as e:
                    self.error_ids.append(self.required_ids[i])


            else:
                url = "https://www.pagesjaunes.fr/pros/{}".format(self.required_ids[i])
                try:
                    self.extract_details('{}'.format(url))
                except Exception as e:
                    self.error_ids.append(self.required_ids[i])
        ...
        self.column = ["url", "id", "nom_societe", "adresse", "code_postal", "ville", "rubrique", "siret", "dernire_modif", "tva_intra", "principaux_dirigeants", "rating", 'review', "telephone", 'mobile', "fax", "website", "activities", 'prestations', "produits", "description", "horaires", "budget", "tarif_nuit", "cuisine", "ambiance", "formules", "nom_du_chef", "references_et_guides", "moyens_de_paiement", "nbre_etoile_hotel", "nbre_chambres", "marque", "capacite_accueil", "info_pratique", "services_loisirs", "clientele"]
        self.error_column = ["id"]    

        df=pd.DataFrame(self.scraped_data,columns=self.column)
        df.to_csv(self.filename,index=False)  # If excel file of output data is required.
        
        error_df = pd.DataFrame(self.error_ids,columns=self.error_column)
        error_df.to_csv('Error_ids.csv',index=False)
        return df
    ...
    def start(self, args):
        ...
        if args.file_name:
            self.get_data_using_id(args.file_name, args.start_id, args.end_id)
            self.browser.close()
            return "pass"
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--file_name', dest='file_name', type=str, help='file-name to store')
    argparser.add_argument('--start_id', dest='start_id', type=str, help='start id')
    argparser.add_argument('--end_id', dest='end_id', type=str, help='end id')
    ...
    {'':2}--file_name: {args.file_name}
    {'':2}--start_id: {args.start_id}
    {'':2}--end_id: {args.end_id}""")
